...
- 의미 있는 단위가 끝날 때마다 `npm.cmd run validate:data`, `npm.cmd run validate:search`, `npm.cmd run lint`, `npm.cmd run build`를 실행한다.
...
1. 모바일 카드 페이징, 우측 패널, iframe fallback CTA를 실제 화면으로 QA한다.
2. 1차 확장 후 검색 품질을 QA한다.
3. [Workflow Expansion Backlog](./workflow-expansion-backlog.md)의 첫 후보를 `workflow-chain-template.md`로 구체화한다.
4. 후보 체인별 단계에 이론 링크, 검색어, 실행 도구를 추가한다.
5. 메인 대분류 카드와 서비스 바로가기 섹션의 시각 밀도를 재점검한다.
