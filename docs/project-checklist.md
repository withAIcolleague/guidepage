...
1. `codex/add-agriculture-workflow` PR을 merge한다.
2. merge 후 최신 `main`에서 빈 대분류 최소 1개 체인 채우기 완료 여부를 재검증한다.
3. 모바일 카드 페이징, 우측 패널, iframe fallback CTA를 실제 화면으로 QA한다.
4. 1차 확장 후 검색 품질을 QA한다.
5. 중분류별 세부분류 후보와 워크플로우 후보를 보강한다.
