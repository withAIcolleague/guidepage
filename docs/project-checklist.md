...
- 의미 있는 단위가 끝날 때마다 `npm.cmd run validate:data`, `npm.cmd run validate:search`, `npm.cmd run validate:theme`, `npm.cmd run lint`, `npm.cmd run build`를 실행한다.
...
2. [Workflow Expansion Backlog](./workflow-expansion-backlog.md)의 첫 후보를 `workflow-chain-template.md`로 구체화한다.
3. 후보 체인별 단계에 이론 링크, 검색어, 실행 도구를 추가한다.
