...
1. `codex/add-law-policy-workflow` PR을 merge한다.
2. merge 후 최신 `main`에서 농학·생명산업 첫 워크플로우 체인을 추가한다.
3. 빈 대분류 최소 1개 체인 채우기 완료 여부를 점검한다.
4. 모바일 카드 페이징, 우측 패널, iframe fallback CTA를 실제 화면으로 QA한다.
5. 1차 확장 후 검색 품질을 QA한다.
