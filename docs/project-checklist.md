...
1. `codex/detail-ux-qa` PR merge 상태를 확인한다.
2. merge 후 모바일에서 카드 화살표 페이징을 실제 화면으로 QA한다.
3. 우측 상세/검색 패널과 iframe fallback CTA를 QA한다.
4. 자연과학 첫 워크플로우 체인을 추가한다.
5. 의학·보건, 법·정책, 농학·생명산업 순서로 빈 대분류를 채운다.
